...
elForm.addEventListener("submit", function (evt) {
  ...
  if (isNaN(elInput2.value)) {
    elP.textContent = `${elInput1.value} iltimos son kiriting`;
  } 
  else if (elInput2.value < costDubai) {
    elP.textContent = `${elInput1.value} afsuski pulingiz yetmadi`;
  }
  else if(elInput2.value >= costDubai){
    elP.textContent = `${elInput1.value} sayohatga ketishingiz mumkin pullingiz yetdi`;
  }
})